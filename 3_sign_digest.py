...
def enc_digest(filename):
    ...
    # TODO: Task 3-5
    encrypted_message_digest_bytes = public_key.encrypt(message_digest_bytes, padding.OAEP(mgf=padding.MGF1(hashes.SHA256()),algorithm=hashes.SHA256(), label=None, ),)

    # Decrypt the message digest back
    # TODO: Task 3-6
    decrypted_message_digest_bytes = private_key.decrypt(encrypted_message_digest_bytes, padding.OAEP(mgf=padding.MGF1(hashes.SHA256()),algorithm=hashes.SHA256(), label=None, ),)
...
def sign_digest(filename):
    ...
    # TODO: Task 3-7
    signature = private_key.sign(file_data,padding.PSS(mgf=padding.MGF1(hashes.SHA256()),salt_length=padding.PSS.MAX_LENGTH,),hashes.SHA256(),  # Algorithm to hash the file_data before signing
)
